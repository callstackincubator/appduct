...
// The JS half of "compiling Appduct out of production" (see the package README) -- wrapped
// around the symlink-dedup resolver above, not in place of it, so `withAppduct` chains to it
// rather than clobbering it. `APPDUCT_ENABLED=0` is this playground's own opt-out switch for
...
module.exports = withAppduct(config, {
	include: process.env.APPDUCT_ENABLED !== "0",
});
