...
- **Both `android` and `ios-device` require the installed app's id** (issue #63), resolved
  ...
  additionally scans its stdout/stderr for a `/^Error:/mu` line and throws if it finds one, naming
  the package. The value is shape-validated (letters, digits, `.`, `-`, starting with one of them)
  at each use site for two different reasons, not one: on `ios-device` it is the launch argv's
  ...
  grammars (Android forbids a leading/embedded `-` in an `applicationId`; iOS allows one): it is a
  safety check on the argv and the device shell, not a spelling check, so a syntactically safe but
