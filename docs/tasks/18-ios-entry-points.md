...
`feat/native-core`) — `AppductClient` and its `+Session`/`+ToolInvocation` extensions
...
(`packages/native/android`, `packages/appduct`'s scheme discovery) and are out of scope here —
...
`@appduct/react-native` gives a React Native app — register a tool, forward a deep link, read
connection state — without making that app touch `JSONValue`, `ToolDescriptor`'s wire shape, or
`AppductClient`'s actor directly. Issue #48's phase-3 sketch names the shape:

```swift
Appduct.shared.register(name:description:inputSchema:...) { args in ... }
ContentView().onOpenURL { Appduct.shared.handle($0) }
```
...
## The facade (`Sources/AppductCore/Real/AppductAPI.swift` + `Stub/AppductAPIStub.swift`)

`public final class Appduct: Sendable` wraps one `AppductClient` instance. `Appduct.shared`
...
reach a `Appduct` through `.shared`, but `AppductAPITests.swift`
(`@testable import AppductCore`) uses it directly to wrap a scripted `FakeTransportSession`
instead of a real `AppductConnectionManager` — the same pattern
`AppductClientTests.swift` already established for the client itself.

Every method mirrors one already on `AppductClient`, translated across the `[String: Any]`
...
snapshots and one unified `addListener` replacing `AppductClient`'s three separate
...
matching `@appduct/react-native`'s `registerTool(...).remove()` — an app that drops the handle
without calling `.remove()`/`.cancel()` keeps the registration/subscription alive, on purpose.

`Stub/AppductAPIStub.swift` mirrors the same public API as inert no-ops, exactly like
`Stub/AppductClientStub.swift` does for the client itself — see Decision 4 for the one place its
...
1. **The `[String: Any]` boundary converts by the same rules `JSONSerialization` uses, but throws
   ...
   facade's own `jsonValue(fromFoundation:)` (private to `AppductAPI.swift`) is structurally
   identical but throws on that case instead; a tool handler's return value that fails this
   conversion is reported as `AppductToolHandlerError(type: "tool_serialization_error", ...)` —
   the exact wire error type and message text `AppductClient+ToolInvocation.swift`'s
...
3. **`Appduct.shared`'s first access starts `restoreSession()` in the background, not on the
   ...
   `AppductClient.init` itself defers its own transport/foreground wiring to a `Task` for exactly
   ...
   `await Appduct.shared.restoreSession()` itself — idempotent, since the underlying
   `AppductClient.restoreSession()` is a no-op once `heldSession` or `clientState` has already
   moved past idle/closed.
4. **The stub facade's `state` is `.closed`, not `.idle`.** `Stub/AppductClientStub.swift`
   answers `.idle` for the same reason `AppductConnectionManager`'s own stub does — a stub build
   ...
   client stub's own choice, documented at the call site (`Appduct.state`'s doc comment) rather
   than silently inconsistent.
5. **`ClientState` and `BuildConfig` are new public names, not the existing internal/public types
   renamed.** `AppductClientState` is already `public` and used by the RN bridge
   ...
   `public typealias ClientState = AppductClientState` instead. `AppductBuildConfig`
   (`AppductConnectionManager.swift`) is `internal`, not `public` — a `public typealias` cannot
   point at an internal type, so `BuildConfig` is a small new public struct with the same three
   fields, populated from `currentAppductBuildConfig()` (itself internal, callable directly since
   `AppductAPI.swift` lives in the same module).
...
7. **The playground targets iOS 15.1** — matching `Package.swift`'s `.iOS("15.1")` platform floor
   for `AppductCore` itself, so the example app never claims API availability the SDK's own
...
8. **No `AppductTrust`/`AppductCliPins` in the playground's `Info.plist`.** Unlike
   ...
   `appduct link` puts on the deep link for that session (`trust: "link"`).
...
Run from the worktree root (`/Users/szymon.chmal/Projects/appduct/.claude/worktrees/agent-a0e98193bb5e2cd5a`)
unless noted.

- **`swift build -c debug`, `swift build -c release`, `swift build -c release --traits
  AlwaysEnabled`** — all pass, each with the facade's new files (`AppductAPI.swift`,
  `AppductAPIStub.swift`) compiled into the module.
- **`swift test`** — 120/120 pass: the 108 pre-existing tests unchanged, plus 12 new
  `AppductAPITests` covering exactly the list this task's brief named: register → snapshot
...
- **`appduct doctor <built .app>`** against both configurations of the playground:
  - Debug: `Present true`, `Signals ios-core-marker-symbol, ios-info-plist-keys`,
    ...
    that signal is `RCTNativeAppduct`, an RN-only class; this app has no RN bridge at all, and
    ...
    strings `AppductCliPins`/`AppductTrust`/`AppductAllowPrivateLanOnly` anywhere in the
    binary, and the real `AppductConnectionManager` compiles those exact string constants in
...
- **End-to-end in the iOS Simulator — completed**, unlike the two earlier attempts recorded in
  ...
  - Private daemon: `APPDUCT_STATE_DIR=/tmp/appduct-3a-state`, `config.json` with
  ...
  - `appduct link --scheme appduct-native --open ios-sim --device
  ...
  - **Discovery, not a defect:** `xcrun simctl openurl` on this Simulator/iOS version presents an
    ...
    *did* respond to the control tool's own device-space `tap` normally (converting the tool's
    ...
    simulator-automation wrinkle, not something `Appduct.shared.handle(_:)` needs to account
    for.
  - Once dismissed: `state -> connecting` then `state -> active`, `session -> <id> as iphone` —
    ...
    `appduct ls`.
  - `appduct tools` → all five (`sum`, `call_count`, `reset_counter`, `slow_task`,
    `throwing_tool`), matching `playground/app/(tabs)/index.tsx` name-for-name.
  - `appduct invoke sum --input '{"a":2,"b":3}'` → `{"total": 5}`.
  - `appduct invoke call_count --input '{}'` → `{"count": 1}` (bumped by the `sum` call above).
  - `appduct invoke throwing_tool --input '{}'` → `tool_execution_error`,
    `"throwing_tool always fails on purpose."` — the app's own thrown `Error`, translated through
    the core's generic (non-`AppductToolHandlerError`) failure path.
  - `appduct invoke reset_counter --input '{}'` → `{"count": 0}`.
  - `appduct invoke slow_task --input '{}'` → `{"done": true}` after ~1.6s (progress reports not
  ...
  - Tapped "Post playground_button_tap" in the app (in-app tap, device-space coordinates, no
    macOS-side workaround needed); `appduct events --since 0` shows
  ...
  - Background/foreground: pressed the Simulator's Home button
    (`mcp__Claude_Code_iOS_Simulator__control` `button` action) → `appduct ls` reported the
    ...
    `Appduct.shared`'s first access on the new process called `restoreSession()` in the
    ...
    `AppductClientTests`' `testRestoreSessionStartsResumeFromAValidLease` and this task's own
  ...
  - Cleaned up: `appduct daemon stop`, `xcrun simctl terminate`, detached the Simulator control
    panel.
...
- CocoaPods trunk / SwiftPM tag publishing for `AppductCore` (named in the issue's phase-3 scope)
  ...
  already single-sourced from `packages/react-native/package.json` (`AppductCore.podspec`).
