...
Today that pattern is awkward: `if (__DEV__) useAppductTool(...)` is a rules-of-hooks
...
`packages/react-native/src/useAppductTool.ts` (and the `createUseAppductTool` factory
shared with `noop.ts`, so both entries keep identical shape):

```ts
useAppductTool(definition, deps?, options?: { enabled?: boolean })
```
...
- Show the **recommended predicate**: an app-owned build flag inlined by the bundler, e.g.
  `process.env.EXPO_PUBLIC_APPDUCT_TOOLS === "full"`.
