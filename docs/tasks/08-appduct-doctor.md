# 08 — `appduct doctor`: artifact-level inclusion check
...
contain Appduct native code, and fail the build if it's wrong.
...
```
appduct doctor <path-to-artifact> [--assert-absent | --assert-present] [--json]
```

- Accept `.app`, `.ipa`, `.apk`, `.aab`. Report per-platform: Appduct native code
...
Inspecting the JS bundle for the Appduct JS half. Worth doing eventually — the Metro swap
...
- A missing external tool produces a distinct exit code and message, and is covered by a
  test.
- Documented in `packages/appduct/README.md` and `docs/CI.md` as a **release-gate step**,
