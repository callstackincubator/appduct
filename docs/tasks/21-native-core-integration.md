...
where every Decision and Phase bullet in [issue #48](https://github.com/callstackincubator/appduct/issues/48)
...
| --- | --- | --- | --- |
| 1 | Session logic single-sourced in native (reconnect, grace, lease restore, registry snapshots/deltas, timeouts, cancel, progress live in Swift/Kotlin once, shared by RN and native consumers) | `packages/native/ios/Sources/AppductCore/Real/AppductClient*.swift`, `packages/native/android/core/src/main/java/.../AppductClient.kt` (`docs/tasks/15-native-session-logic.md`, `docs/tasks/16-android-session-logic.md`) | **Done** |
| 2 | Release exclusion structural per platform, always fails closed (Android two artifacts; iOS CocoaPods `:configurations`; iOS SwiftPM define + trait; no runtime checks anywhere) | Android: `packages/native/android/core`/`core-noop`, `android/build.gradle`'s `java.srcDirs` swap (RN) / `debugImplementation`+`releaseImplementation` (plain app). iOS CocoaPods: `packages/native/ios/AppductCore.podspec` + `packages/react-native/Appduct.podspec`'s `:configurations => ['Debug']`. iOS SwiftPM: `Package.swift`'s `.when(configuration: .debug)` + `AlwaysEnabled` trait (`docs/tasks/14-native-core-extraction.md`) | **Done**, with the SwiftPM shape changed — see "Known changes" below |
| 3 | `appduct doctor` is the guarantee (marker-symbol-only presence on both platforms; marker never compiled into the no-op/stub; release pipelines run `doctor --assert-absent` as a blocking step) | `AppductCoreMarker` (iOS, `docs/tasks/14-native-core-extraction.md`), `AppductNativeMarker` (Android, pre-existing, unchanged). CI: `.github/workflows/test.yaml`'s `android`/`ios` jobs, now including the native playground legs added by this task | **Done** |
| 4 | Schemas on native are raw JSON Schema (`[String: Any]`/`JSONObject`); no app-side input validation on native | `AppductToolDescriptor`/`AppductToolRegistry` on both platforms; `Appduct.register`'s `inputSchema`/`outputSchema: [String: Any]?`/`JSONObject` (`docs/tasks/15-native-session-logic.md`, `docs/tasks/18-ios-entry-points.md` Decision 2) | **Done** |
| 5 | Public JS API unchanged (`useAppductTool`, `registerTool`, `postEvent`, the listener surface); only the TurboModule spec shrinks | `packages/react-native/src/public-api.ts`, `NativeAppduct.ts` (frozen spec, `docs/tasks/15-native-session-logic.md`) | **Done** |

## Phases (issue #48)

| Phase | What it covers | Where it landed | Status |
| --- | --- | --- | --- |
| 0 | Fail-closed autolinking fallback (`APPDUCT_ENABLED` unparseable → `devOnly`, not `everyBuild`) | `packages/react-native/react-native.config.js` (pre-`feat/native-core`, per `docs/tasks/14-native-core-extraction.md`'s own header: "fixed and committed before any of the rest of this work") | **Done** |
...
| 3 | Native entry points and packaging: `Appduct.shared`/`Appduct` facades, native playgrounds, `appduct init`/scheme discovery, publishing | `docs/tasks/18-ios-entry-points.md`, `docs/tasks/19-android-entry-points.md`, `docs/tasks/20-cli-native-scheme-discovery.md`, `playground-native/{ios,android}` | **Done except publishing** — see below |
| — | Publish `AppductCore` to CocoaPods trunk / a SwiftPM tag, `core`/`core-noop` to Maven Central | Not done | **Ops task, deliberately not performed** (below) |
...
- **The second SwiftPM product became the `AlwaysEnabled` trait.** The issue's Decision 2 reads
  as if `AppductCoreAlways` would be a second SwiftPM *product*. A target's sources — and
  therefore which `#if APPDUCT_ENABLED` branch is active — are shared by every product built
...
- **Vendoring by copy, not a published-artifact dependency.** `@appduct/react-native` copies
...
- **`AppductConnectionManager.kt`'s hidden RN dependency, removed.** The issue's own text
  ...
  bridge (`NativeAppductModule.kt`, via `.toHashMap()`). `docs/tasks/14-native-core-extraction.md`.
- **`sessionChange` carries only `{ sessionId, alias }`.** The frozen TurboModule spec's
  `AppductSessionChangeEventNative` dropped the pre-port `{ type, sessionId, alias, reason? }`
...
- **`defaultToolTimeoutMs` is no longer app-configurable.** Timeout enforcement moved entirely to
  native, fixed at `APPDUCT_DEFAULT_TOOL_TIMEOUT_MS` (10 s, matching the old JS default); the
...
- **Facade files excluded from RN vendoring.** `AppductAPI.swift` (iOS) and `Appduct.kt`/
  `AppductInitProvider.kt`/`AppductLinkActivity.kt` (Android) are explicitly filtered out of
  `sync-native-core.mjs`'s copy (`IOS_FACADE_FILES`/`ANDROID_FACADE_FILES`) — an RN app must never
  end up with a second `AppductClient` behind a vendored facade racing the RN bridge's own
...
- **CI** (`.github/workflows/test.yaml`): `ios` job now runs `xcodegen generate` +
  ...
  `appduct doctor` gate; `android` job now runs `:app:assembleDebug :app:assembleRelease` for
...
1. **Kotlin cancellation acknowledgement never reached the wire.**
   `AppductToolInvoker.kt`'s `handleToolCancel`/`abortAllInFlight` cancel the handler's own
   coroutine `Job`, so by the time the `catch (CancellationException)` branch's `sendToolError`
   suspended through `AppductClient.rawSend`'s `suspendCancellableCoroutine`, the coroutine was
   ...
   `withContext(NonCancellable) { ... }`. `FakeAppductTransport` gained a `deferSendCompletion`
...
2. **`sessionChange` lost `type`/`reason` — a public-API regression against `main` and against issue
   #48 decision 5.** Restored end to end: `NativeAppduct.ts` (the one sanctioned edit to the
   frozen TurboModule spec, recorded in its own header comment), `Appduct.types.ts`/
   `client-types.ts`/`AppductModule.ts`/`client/index.ts`, the Swift core (`AppductClientTypes.swift`'s
   new `AppductSessionChangeKind`, emitted from `onAckReceived`/`finalizeSessionLost`/`disconnect`)
   and its bridge (`AppductTurboBridge.swift` + `RCTNativeAppduct.mm`), the Kotlin core
   (`AppductClient.kt`, the `Appduct` facade's new `SessionChangeType`, mirrored in
   `core-noop`) and its bridge (`NativeAppductModule.kt`). Tests extended in all three suites for
...
3. **Neither native core clamped a declared tool `timeout_ms`.** The old JS registry clamped via
   ...
   both the local abort timer and the wire value; ported to `AppductToolRegistry.upsert` on both
   platforms (still rejecting non-positive/non-integer values per PROTOCOL.md §5), with identically
   named bounds constants on both platforms pointing at the TS source, and `AppductClient.swift`'s
...
4. **JS `postEvent` silently demoted every native rejection to `logger.debug`.** Restored the old
   ...
   `E_APPDUCT_NOT_ACTIVE` rejection code both bridges now use (iOS via the core's existing
   `AppductNotActiveError`; Android via a state guard in `NativeAppductModule`, since the
   Kotlin core's own `postEvent` stays a silent best-effort no-op for plain-app callers).
5. **`AppductModule.web.ts`'s `restoreSession`/`disconnect` didn't throw**, contradicting the
...
6. **Android descriptor/connect-input JSON parsing lived only in the RN bridge**, unlike the Swift
   equivalents already vendored in the core. Moved to `AppductToolDescriptor.fromJson`/
   `AppductConnectInput.fromJson` in `packages/native/android/core`'s `AppductClientTypes.kt`,
...
7. **IPv6 literals were never bracketed** in either `AppductConnectionManager`'s daemon connect
   URL, unlike `formatAgentWebSocketUrl` (`packages/shared/src/domains/transport.ts`). Factored into
   a small pure `formatAppductWebSocketUrl` function on each platform (same name in both Swift
...
8. **`artifact-inspect.ts`'s Android marker comment was stale**, still pointing at
   ...
   `packages/native/android/core/.../AppductNativeMarker.kt` and its `sync-native-core.mjs`
   vendored copy.
9. **Not done (optional, judged non-trivial): `sync-native-core.mjs`'s filename-based facade
   exclusion left as a list on both platforms.** A directory split (`Sources/AppductCore/API/`)
   would work for iOS but touches `Package.swift`'s target paths, `AppductCore.podspec`, and the
...
- The pre-existing `configuredPins` TLS-delegate race in `AppductConnectionManager.swift` (a
...
Run from this worktree (`/Users/szymon.chmal/Projects/appduct/.claude/worktrees/agent-a3ad3c9bae8781450`)
