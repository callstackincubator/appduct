...
/**
 * Raised when the daemon this client reached is running a different Appduct version and could
...
export class DaemonVersionMismatchError extends Error {
  ...
  constructor(
    ...
    super(
      message ??
        `The running Appduct daemon is version ${daemonVersion}, but this client is version ${clientVersion}. ` +
          `It was not restarted automatically because ${describeLiveState(live)} would be lost. ` +
          `Run "appduct daemon stop" once that state is expendable, or force it with "--daemon-restart".`,
    );
...
export class DaemonVersionRestartIneffectiveError extends DaemonVersionMismatchError {
  constructor(daemonVersion: string, clientVersion: string, live: DaemonLiveState) {
    super(
      ...
      clientVersion,
      live,
      `The Appduct daemon was restarted to match this client (version ${clientVersion}), but the daemon now answering still reports version ${daemonVersion}. ` +
        `Either the old daemon never exited (compare "daemon.pid" with what "appduct daemon status" reports), ` +
        `or another process spawned an older daemon in the meantime. ` +
        `Run "appduct daemon stop", then retry.`,
    );
 ...
 * server's startup stream; deliberately *not* passed by `appduct/client`, whose callers are
...
export type VersionCheckOptions = {
   ...
   * `APPDUCT_DAEMON_RESTART` env var, or `config.json`'s `restartDaemonOnVersionMismatch`). */
...
const sendRequest = <TResult>(
  ...
  return new Promise<TResult>((resolve, reject) => {
    ...
    socket.on("close", () => {
      finish(() =>
        reject(new DaemonUnavailableError("Connection to the Appduct daemon closed.", "closed")),
      );
    });
  });
};
...
const defaultSpawn: SpawnFn = async (args, context) => {
  ...
  // afterwards the daemon holds the fd for its whole life. `appduct daemon start` spawns
  ...
  try {
    ...
    const childEnv: NodeJS.ProcessEnv = { ...process.env, APPDUCT_STATE_DIR: context.stateDir };
    // A daemon this client spawns must report its real version, whatever this process inherited.
    // Otherwise a stray `APPDUCT_DAEMON_VERSION_OVERRIDE` in the operator's shell would be
...
const spawnDaemonAndWait = async (
  ...
  if (!ready) {
    ...
    const lockHint = (await pathExists(paths.spawnLockPath))
      ? ` Another process holds the spawn-lock at "${paths.spawnLockPath}"; if no "appduct daemon" is starting, delete it and retry.`
      : "";

    throw new DaemonUnavailableError(
      `Timed out waiting for the Appduct daemon socket at "${paths.socketPath}".${lockHint}`,
    );
  }
...
const readStatusIfReachable = async (
  ...
  try {
    ...
    if (isRequestTimeoutError(error)) {
      throw new DaemonUnavailableError(
        `The Appduct daemon at "${context.paths.socketPath}" did not answer "daemon.status" within ${RESTART_REQUEST_TIMEOUT_MS}ms, so it was not replaced. ` +
          `It is running but unresponsive — run "appduct daemon stop" and retry.`,
        "timeout",
      );
    }
...
const restartDaemon = async (context: VersionCheckContext): Promise<void> => {
    ...
    context.pollIntervalMs,
    {
      ...
      beforeSpawn: async () => {
        ...
        try {
          ...
          if (isRequestTimeoutError(error)) {
            throw new DaemonUnavailableError(
              `The Appduct daemon at "${context.paths.socketPath}" did not acknowledge "daemon.shutdown" within ${RESTART_REQUEST_TIMEOUT_MS}ms, so it was left running. ` +
                `Run "appduct daemon stop" and retry.`,
              "timeout",
            );
          }
        ...
        if (!(await pollUntilDaemonGone(context.paths, context.waitTimeoutMs, context.pollIntervalMs))) {
          throw new DaemonUnavailableError(
            `The Appduct daemon at "${context.paths.socketPath}" did not shut down in time; run "appduct daemon stop" and retry.`,
          );
        }
  ...
  if (!outcome.ownedLock && outcome.lockWaitTimedOut && (await readPidfilePid(context.paths)) === pidBefore) {
    ...
    throw new DaemonUnavailableError(
      `The Appduct daemon at "${context.paths.socketPath}" did not shut down in time: another process has held the spawn-lock at ` +
        `"${context.paths.spawnLockPath}" for longer than this command waited. Retry, or run "appduct daemon stop" if nothing is starting a daemon.`,
    );
  }
};
...
const warnOlderClient = (context: VersionCheckContext, daemonVersion: string): void => {
  const message =
    `appduct: the running daemon is version ${daemonVersion}, newer than this client (${context.check.clientVersion}). ` +
    ...
    `Run "appduct daemon stop" if you want this client's daemon instead.\n`;
...
const runVersionCheck = async (context: VersionCheckContext): Promise<void> => {
  ...
  if (replacement === undefined) {
    throw new DaemonUnavailableError(
      `The Appduct daemon was restarted but nothing is listening at "${context.paths.socketPath}"; check "${context.paths.logFilePath}".`,
    );
  }
...
export const openDaemonStream = async (
  ...
  socket.on("close", () => {
    for (const [, pending] of pendingCalls) {
      clearTimeout(pending.timeout);
      pending.reject(new DaemonUnavailableError("Connection to the Appduct daemon closed.", "closed"));
    }
