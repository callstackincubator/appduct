 ...
 * `createTlsManager` auto-generates one (same material/hygiene as `appduct keygen`, via
 * `key-material.ts`) so a first-time `appduct daemon` needs no manual keygen step, and reports
...
import type { AgentEndpoint } from "@appduct/shared";
...
export const loadHostKeyPem = async (keyPath: string): Promise<string> => {
  ...
  if ((fileStat.mode & 0o077) !== 0) {
    throw new HostKeyError(
      `Appduct host key at "${keyPath}" is group- or world-readable (mode ${(fileStat.mode & 0o777).toString(8)}). ` +
        `Refusing to load it — run "chmod 600 ${keyPath}" or re-run "appduct keygen".`,
    );
  }
...
export const loadOrGenerateHostKeyPem = async (keyPath: string, warn?: TlsWarnFn): Promise<string> => {
  ...
  warn?.(
    `Appduct: no host key found at "${keyPath}" — generated one (dev mode). ` +
