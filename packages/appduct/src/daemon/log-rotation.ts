 ...
 * (`rpc/client.ts`'s `defaultSpawn`, which is also what `appduct daemon start` spawns through),
...
export const rotateDaemonLogIfNeeded = async (
  ...
  try {
    ...
    if (held) {
      warn(
        `appduct: "${options.logFilePath}" is over its size cap but a daemon still holds it; leaving it in place.`,
      );
    ...
    try {
      ...
      warn(`appduct: rotated "${options.logFilePath}" but could not set 0600 on "${backupPath}": ${(error as Error).message}`);
    }
    ...
    warn(`appduct: failed to rotate "${options.logFilePath}": ${(error as Error).message}`);
...
export const ensureDaemonLogMode = async (
  ...
  try {
    ...
    warn(`appduct: could not set 0600 on "${logFilePath}": ${(error as Error).message}`);
    return false;
  }
};
