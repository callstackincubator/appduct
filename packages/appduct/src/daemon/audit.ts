...
export const createAuditLogger = (options: AuditLoggerOptions): AuditLogger => {
  ...
  const pruneOnce = async (): Promise<AuditPruneResult> => {
    ...
    if (listing.failed) {
      ...
      failedPrunes += 1;
      warn(`appduct daemon: failed to read the audit directory to prune it: ${listing.error?.message}`);
    ...
    for (const file of listing.files) {
      ...
      try {
        ...
        failed += 1;
        failedPrunes += 1;
        warn(`appduct daemon: failed to prune the audit record "${file.name}": ${(error as Error).message}`);
      }
    }
  ...
  const record = (entry: Omit<AuditRecord, "ts">): void => {
    ...
    queue = queue.then(async () => {
      try {
        ...
        failedWrites += 1;
        warn(`appduct daemon: failed to write an audit record: ${(error as Error).message}`);
      }
    });
  };
  ...
  const prune = (): Promise<AuditPruneResult> => {
    ...
    queue = queue.then(async () => {
      try {
        ...
        failedPrunes += 1;
        warn(`appduct daemon: failed to prune the audit log: ${(error as Error).message}`);
