...
export type CreateMcpServerOptions = {
   ...
   */
  checkVersion?: VersionCheckOptions;
  /** The scheme composing `appduct_connect`'s deep link, already resolved by `commands/mcp.ts`
   * against the shared order in `scheme.ts` (`--scheme` > `APPDUCT_SCHEME` > project
   * `.appduct/config.json` > state `config.json` > `app.json`). May legitimately be undefined —
   * the server starts either way and only `appduct_connect` needs one. */
  scheme?: string;
...
export const createMcpServer = async (options: CreateMcpServerOptions): Promise<McpServerHandle> => {
  ...
  try {
    ...
    if (error instanceof DaemonVersionMismatchError) {
      ...
      console.error(`appduct mcp: refusing to start. ${error.message}`);
    }
        ...
        if (event.kind === "tool_call_progress" && callId !== undefined) {
          ...
          if (data.callId === callId) {
            sendNotification({
              ...
              console.error("appduct mcp: failed to send a progress notification:", error);
            });
          }
        }
      });
      ...
      resources: [
        {
          ...
          name: "sessions",
          description: "Live Appduct device sessions (sessions.list passthrough), as JSON.",
