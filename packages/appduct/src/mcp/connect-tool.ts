/**
 * The built-in `appduct_connect` / `appduct_wait_for_session` management tools
...
export const CONNECT_TOOL_NAME = "appduct_connect";
export const WAIT_FOR_SESSION_TOOL_NAME = "appduct_wait_for_session";
 ...
 * about it, and went straight to `appduct_wait_for_session` — which then blocked for its full
...
const QR_INSTRUCTIONS =
  ...
  "appduct_wait_for_session: show the user the \"qr\" field verbatim inside a fenced code block " +
  ...
  "device. Only then call appduct_wait_for_session; it blocks until the device connects and " +
...
export const WAIT_FOR_SESSION_TOOL_DESCRIPTOR = {
  ...
  description:
    "Wait for a session minted by appduct_connect to be claimed by a device. Resolves as soon " +
...
export const handleConnectTool = async (
  ...
  // leave `appduct_wait_for_session` blocking for its whole timeout with nothing to explain why.
...
const daemonClosedMessage = (sessionId: string): string =>
  `The connection to the Appduct daemon closed while waiting for session "${sessionId}".`;

const daemonUnreachableMessage = (sessionId: string, error: unknown): string =>
  `The Appduct daemon is unreachable, so session "${sessionId}" cannot be waited on${describeCause(error)}.`;
    ...
    return await new Promise<WaitForSessionToolResult>((resolve, reject) => {
      ...
      stream
        ...
        .then(async () => {
          ...
          // appduct_connect), or in the round trip the subscribe itself took. Either way the
