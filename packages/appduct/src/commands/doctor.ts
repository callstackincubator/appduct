...
export const handleDoctorCommand = async (
  ...
  if (!options.artifactPath) {
    throw usageError("Usage: appduct doctor <path-to-artifact> [--assert-present | --assert-absent]");
  }
  ...
  if (options.assertPresent && !inspection.present) {
    throw assertionError(
      `Expected Appduct to be present in "${options.artifactPath}", but no inclusion signal was found.`,
      data,
    );
  }
  ...
  if (options.assertAbsent && inspection.present) {
    throw assertionError(
      `Expected Appduct to be absent from "${options.artifactPath}", but found: ${inspection.signals.join(", ")}.`,
      data,
    );
  }
