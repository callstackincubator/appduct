/**
 * `appduct mcp` (ARCHITECTURE.md §9, §10): starts a stdio MCP server that proxies the daemon RPC
 ...
 *
 * The deep-link scheme (`--scheme`, `APPDUCT_SCHEME`, project/state config, `app.json` — see
 ...
 * `APPDUCT_SCHEME` is a typo the operator wants to hear about immediately, not only if and when
 * some agent happens to call `appduct_connect`. Stderr, never stdout: stdout carries MCP
...
const resolveSchemeForServer = async (
  ...
  try {
    ...
    (context.stderr ?? process.stderr).write(
      `appduct mcp: could not resolve a deep-link scheme: ${message}\n` +
        "The server is starting anyway; appduct_connect will fail until this is fixed.\n",
    );
...
export const handleMcpCommand = async (context: McpCommandContext): Promise<McpHostedResult> => {
  ...
  handle.server.onerror = (error: Error) => {
    console.error("appduct mcp: transport error:", error);
  };
