 ...
 * that were given no explicit target (notably `appduct_connect` over MCP) can deliver to the one
 ...
 * CLI, the shared `mintLink` core and `appduct_connect` each render in their own wording. */
...
const runDevicectlJson = async (
  ...
  try {
    dir = await mkdtemp(path.join(tmpdir(), "appduct-devicectl-"));
  } catch (error) {
 ...
 * forbids `-` but allows `_`, as in `com.my_company.app`; iOS is the other way round), so this
 * pattern accepts the union of both rather than picking one platform's spelling rules to enforce on
 * the other. That makes it a safety check
...
export const invalidAppIdMessage = (value: string): string =>
  `"${value}" is not a valid app id (letters, digits, ".", "_" and "-", starting with a letter ` +
  "or digit).";
...
const deliverIosDevice = async (
  ...
  await run(exec, "xcrun", [
    ...
    // running instance first, which Appduct copes with because a delivered link supersedes a
    // held session.
 ...
 *
 * The other two prefixes cover the launch being *refused* rather than unresolved — typically a
 * `SecurityException` for an activity that is not exported. Current `am` reports that as
 * "Exception occurred while executing 'start':" and exits non-zero (which `run` already turns into
 * an error), but an `adb` without the shell protocol drops the remote exit status, so the output is
 * the only reliable signal there too. "Warning: Activity not started, intent has been delivered to
 * currently running top-most instance" is deliberately *not* matched: that is a `singleTask` app
 * (React Native's default) receiving the link in `onNewIntent`, i.e. a successful delivery.
 */
const AM_START_ERROR_PATTERN = /^(?:Error:|Exception occurred while executing|Security exception:)/mu;
...
export type OpenTargetOptions = {
  target: OpenTarget;
  /** The composed `<scheme>:///?appduct=<payload>` deep link to deliver. */
  deepLink: string;
