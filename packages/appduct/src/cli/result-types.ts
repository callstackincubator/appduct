...
/**
 * `appduct init` (issue #29). `created`/`changed` are what make the command's idempotency
...
export type InitCommandData = {
   ...
   * be read as `scheme.ts`'s resolution tiers: `init` never consults `APPDUCT_SCHEME` and
...
export type LinkCommandData = {
  sessionId: string;
  /** Already includes the `pin` query param (see `pin` below) alongside `appduct`. */
  deepLink: string;
