...
const makeDir = async (): Promise<string> => {
  const directory = await mkdtemp(path.join(tmpdir(), "appduct-scheme-"));
 ...
 * `$HOME` or `~/.appduct` of whatever machine runs the suite.
 */
...
const writeProjectConfig = async (dir: string, value: unknown): Promise<string> => {
  const configPath = path.join(dir, ".appduct", "config.json");
...
describe("findProjectConfig", () => {
  ...
  // `<home>/.appduct` is the *default state dir*, and essentially every project lives under the
  ...
  test("never treats <home>/.appduct as a project config, even with an unrelated state dir", async () => {
    ...
    expect(findProjectConfig(projectDir, await isolatedLookup())).toBe(
      path.join(home, ".appduct", "config.json"),
    );
  });
...
describe("readProjectConfigScheme", () => {
  ...
  ])("throws for %s — this is Appduct's own file", async (_label, value) => {
    const dir = await makeDir();
...
describe("resolveScheme precedence", () => {
  ...
  test("2. APPDUCT_SCHEME wins when there is no flag", async () => {
  ...
  test("4. the state dir config wins over app.json", async () => {
    const { cwd, stateDir } = await makeFullyLoadedRoot();
    await rm(path.join(cwd, ".appduct"), { force: true, recursive: true });
  ...
  test("reports every location tried, in order, when nothing resolves", async () => {
    ...
    expect(tried).toEqual([
      "the --scheme flag",
      "the APPDUCT_SCHEME environment variable",
      `.appduct/config.json (searched upwards from ${dir})`,
...
describe("describeMissingScheme", () => {
  test("numbers the locations and names every way to set one", () => {
    ...
    expect(message).toContain("APPDUCT_SCHEME");
    expect(message).toContain("appduct init");
  });
