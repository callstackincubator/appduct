 ...
 * Here a real daemon subprocess is started with `APPDUCT_DAEMON_VERSION_OVERRIDE` so it reports
...
/**
 * Reads status through a real `appduct daemon status --json`. That command is deliberately
...
/** Starts a real `appduct daemon run` subprocess reporting `version` (default: an older one). */
const startStaleDaemon = async (stateDir: string, version: string = STALE_VERSION): Promise<number> => {
  const proc = spawnCli(["daemon", "run"], stateDir, {
    APPDUCT_DAEMON_VERSION_OVERRIDE: version,
  });
...
describe("e2e: daemon/CLI version drift", () => {
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(lsResult.error?.message).toContain("appduct daemon stop");
  ...
  test(
    ...
    async () => {
      ...
      expect(status.warning).toContain("appduct daemon stop");
