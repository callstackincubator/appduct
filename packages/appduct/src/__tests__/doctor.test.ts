...
describe("doctor command", () => {
  ...
  test("--assert-present holds when Appduct is present", async () => {
    const root = await withFixtureRoot();
    const apkPath = path.join(root, "included.apk");
    await buildZipFixture(apkPath, {
      "classes.dex": "Lcom/callstack/appduct/X;Lcom/callstack/appduct/AppductNativeMarker;",
  ...
  test("--assert-present throws assertion_error when Appduct is absent", async () => {
    const root = await withFixtureRoot();
  ...
  test("--assert-absent holds when Appduct is absent", async () => {
    const root = await withFixtureRoot();
    const apkPath = path.join(root, "excluded.apk");
    await buildZipFixture(apkPath, {
      "classes.dex": "Lcom/example/app/MainActivity;",
  ...
  test("--assert-absent throws assertion_error when Appduct is present", async () => {
    const root = await withFixtureRoot();
    const apkPath = path.join(root, "included.apk");
    await buildZipFixture(apkPath, {
      "classes.dex": "Lcom/callstack/appduct/X;Lcom/callstack/appduct/AppductNativeMarker;",
