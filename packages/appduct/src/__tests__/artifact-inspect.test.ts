...
describe("artifact-inspect: iOS .ipa (synthetic zip fixture)", () => {
  test("included: reports present with the objc-class-symbol signal", async () => {
    ...
    await buildZipFixture(ipaPath, {
      "Payload/Fixture.app/Fixture": Buffer.concat([
        ...
        Buffer.from("RCTNativeAppduct"),
        Buffer.from(" junk-mach-o-bytes-after"),
      ]),
      "Payload/Fixture.app/Info.plist": "not appduct related",
    });
  ...
  test("included via AppductCoreMarker alone (primary signal) reports present", async () => {
    const root = await withFixtureRoot();
  ...
  test("Info.plist keys alone (corroborating signal only) does NOT prove presence -- an app can author these keys by hand without the real implementation being present", async () => {
    ...
    await buildZipFixture(ipaPath, {
      ...
      "Payload/Fixture.app/Info.plist": "...AppductTrust...link...",
    });
...
describe("artifact-inspect: iOS .app (unpacked directory fixture)", () => {
  test("included: reports present", async () => {
    ...
    await buildAppDirectoryFixture(appPath, {
      Included: Buffer.concat([Buffer.from("prefix"), Buffer.from("RCTNativeAppduct"), Buffer.from("suffix")]),
...
describe("artifact-inspect: Android .apk (synthetic zip fixture)", () => {
  test("dex-package-symbol alone (no keep-rule marker) is reported but does not prove presence -- a no-op stub compiled at the real package's FQCN (android/build.gradle's APPDUCT_ENABLED-gated source-set swap) triggers this same signal", async () => {
    const root = await withFixtureRoot();
  ...
  test("included via the AppductNativeMarker keep-rule symbol alone (primary signal, synthetic: see artifact-fixtures.ts and the task report for what a real R8 build additionally verifies)", async () => {
    const root = await withFixtureRoot();
  ...
  test("excluded: reports absent with no signals", async () => {
    ...
    await buildZipFixture(apkPath, {
      ...
      "AndroidManifest.xml": "no appduct keys in here",
    });
