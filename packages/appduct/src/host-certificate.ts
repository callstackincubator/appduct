...
const resolveEcHash = (curve: string): webcrypto.AlgorithmIdentifier => {
  switch (curve) {
    ...
    case "P-521":
      return "SHA-512";
    default:
      throw new Error(`Unsupported EC named curve "${curve}" for Appduct host certificates.`);
  }
};
...
const importSigningKeyPair = async (keyPem: string): Promise<ImportedKeyPair> => {
  ...
  if (jwk.kty === "RSA") {
    ...
    if (typeof modulusLength !== "number" || typeof jwk.e !== "string") {
      throw new Error("Appduct could not determine RSA key details for certificate generation.");
    }
  ...
  if (jwk.kty === "EC") {
    ...
    if (typeof namedCurve !== "string") {
      throw new Error("Appduct could not determine the EC named curve for certificate generation.");
    }
  ...
  throw new Error(`Appduct does not support "${jwk.kty ?? privateKeyObject.asymmetricKeyType}" host keys.`);
};
