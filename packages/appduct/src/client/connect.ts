 ...
 * selector fails fast with a typed {@link AppductError} rather than surfacing on the first
 * `call()`.
 */
import { RPC_METHODS, type SessionsDescribeResult } from "@appduct/shared";
...
export type ConnectOptions = {
  ...
  selector?: string;
  /** Defaults to `APPDUCT_STATE_DIR`, else `~/.appduct` — the same resolution the CLI uses. */
  stateDir?: string;
