 ...
 */
import { RPC_METHODS, type EventNotification, type SessionsDescribeResult } from "@appduct/shared";
...
export type LinkOptions = {
   ...
   * `.appduct/config.json`, the state dir's `config.json` and `app.json` (see `scheme.ts`). */
  scheme?: string;
  /** Where scheme discovery starts; defaults to `process.cwd()`. */
  cwd?: string;
  exec?: ExecFn;
  /** The environment handed to `adb`/`simctl`, *not* the one `APPDUCT_SCHEME` is read from
      ...
      const unsubscribeClose = stream.onClose(() => {
        settle(() =>
          reject(
            new AppductError(
              "connection_error",
              `The connection to the Appduct daemon closed while waiting for session "${sessionId}" to be claimed.`,
            ),
          ),
        );
      });
