 ...
 * parsed defensively:
 *
 * - Android: `app/build.gradle.kts` / `app/build.gradle` for a `manifestPlaceholders["cordieriteScheme"]`
 *   (or `.cordieriteScheme =`) assignment, then `app/src/main/AndroidManifest.xml` for the first
 ...
 * mean `cordierite link` opens the wrong app on whichever platform lost.
 */
...
// --- Android: build.gradle(.kts) "cordieriteScheme" placeholder ---

/**
 * Matches both Groovy DSL (`manifestPlaceholders["cordieriteScheme"] = "myapp"`) and Kotlin DSL
 * (`manifestPlaceholders["cordieriteScheme"] = "myapp"` or the property form
 * `manifestPlaceholders.cordieriteScheme = "myapp"`) — issue #48's sketch shows the bracket form
...
const GRADLE_PLACEHOLDER_PATTERN =
  /manifestPlaceholders\s*(?:\[\s*["']cordieriteScheme["']\s*\]|\.\s*cordieriteScheme)\s*=\s*["']([^"']*)["']/u;
...
const probeAndroidGradle = async (root: string): Promise<NativeSchemeDiscovery> => {
  ...
  for (const candidate of candidates) {
    ...
    if (scheme !== undefined) {
      requireValidNativeScheme(scheme, `"cordieriteScheme" in ${candidate}`);

      return {
        result: {
          source: "android-gradle",
          scheme,
          origin: `"cordieriteScheme" in ${candidate}`,
        },
...
const describeAndroidGradleTried = (candidates: string[], notes: string[]): string => {
  const base = `${candidates.join(", ")} ("cordieriteScheme" manifest placeholder)`;
  ...
  if (distinctSchemes.size > 1) {
    throw usageError(
      "Conflicting deep-link schemes found in this project: " +
        ...
        ". Cordierite will not guess which one is right — pass --scheme <scheme> to pick one.",
    );
  }
