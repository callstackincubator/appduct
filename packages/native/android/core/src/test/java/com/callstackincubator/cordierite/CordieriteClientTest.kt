 ...
 * [CordieriteClient] over a [FakeCordieriteTransport]. Real reconnect/backoff/grace timing (which
 ...
 */
class CordieriteClientTest {
    private fun newClient(): Pair<CordieriteClient, FakeCordieriteTransport> {
        lateinit var fake: FakeCordieriteTransport
        val client =
            CordieriteClient(
                transportFactory = { _, onMessage, onError, onClose ->
                    FakeCordieriteTransport(onMessage, onError, onClose).also { fake = it }
                },
            )
        return client to fake
    }

    /** Waits for a `tool_error` frame among [FakeCordieriteTransport.sentMessages] and returns it --
    ...
    private fun FakeCordieriteTransport.awaitToolError(timeoutMs: Long = 2000): JSONObject {
    ...
    fun `connect resolves and moves to active once session_ack arrives`() =
        runBlocking {
            ...
            assertEquals(CordieriteClientState.active, client.state)
    ...
    fun `registerTool throws for an invalid descriptor`() {
        ...
        assertThrows(CordieriteInvalidToolDescriptorException::class.java) {
            client.registerTool(CordieriteToolDescriptor(name = "bad name", description = "x")) { _, _ -> null }
        }
    }
    ...
    fun `registerTool upsert by name is reflected in registeredTools`() {
        val (client, _) = newClient()
        client.registerTool(CordieriteToolDescriptor(name = "sum", description = "Add.")) { _, _ -> null }
        client.registerTool(CordieriteToolDescriptor(name = "sum", description = "Add two numbers.")) { _, _ -> null }
    ...
    fun `a registered tool call replies tool_result`() =
        runBlocking {
            val (client, fake) = newClient()
            client.connectAndAck(fake)
            client.registerTool(CordieriteToolDescriptor(name = "sum", description = "Add.")) { args, _ ->
    ...
    fun `a thrown handler error replies tool_execution_error`() =
        runBlocking {
            val (client, fake) = newClient()
            client.connectAndAck(fake)
            client.registerTool(CordieriteToolDescriptor(name = "boom", description = "Throws.")) { _, _ ->
    ...
    fun `an unserializable result replies tool_serialization_error`() =
        runBlocking {
            val (client, fake) = newClient()
            client.connectAndAck(fake)
            client.registerTool(CordieriteToolDescriptor(name = "weird", description = "Bad result.")) { _, _ ->
                object : Any() {}
            }
            fake.sentMessages.clear()
    ...
    @Test
    fun `handleUrl returns false for a URL with no cordierite param`() {
        val (client, _) = newClient()
    ...
    fun `handleUrl returns true and reports a bootstrap error for a malformed payload`() =
        runBlocking {
            ...
            assertEquals(true, client.handleUrl("myapp:///?cordierite=not-valid-base64"))
