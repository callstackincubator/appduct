 ...
 */
internal class FakeAppductTransport(
    private val onMessage: (String) -> Unit,
    private val onError: (AppductErrorDetails) -> Unit,
     ...
     * bug (see `AppductToolInvokerTest`/`AppductClientTest`'s cancellation-reaches-the-wire
     * tests). */
    @Volatile var deferSendCompletion = false

    @Volatile var resumeLeaseRecordValue: Map<String, Any?>? = null

    @Volatile var buildConfigValue: AppductBuildConfig = AppductBuildConfig(trust = "pin", hasEmbeddedPins = true, allowPrivateLanOnly = true)
    ...
    override fun getBuildConfig(): AppductBuildConfig = buildConfigValue
    ...
    fun simulateError(details: AppductErrorDetails) = onError(details)
}

/** Polls [condition] on the calling thread until it is true or [timeoutMs] elapses. Used because
 * [AppductClient] confines its state to its own real-time dispatcher (not a virtual-time test
