...
/** Process-memory recovery data owned by [AppductConnectionManager] /
 * [AppductProcessResumeLeaseStore]. Parsed view of the `Map<String, Any?>` returned by
 * [AppductTransport.getResumeLeaseRecord] -- port of `client/resume-lease.ts`'s `ResumeLeaseV1`. */
internal data class AppductParsedResumeLease(
 ...
 * input.
 */
internal fun parseAppductResumeLease(record: Map<String, Any?>?): AppductParsedResumeLease? {
...
internal fun isAppductResumeLeaseExpired(
    lease: AppductParsedResumeLease,
    nowMs: Long,
