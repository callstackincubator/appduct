...
/** A decoded v2 bootstrap payload (PROTOCOL.md §2). Port of `@cordierite/shared`'s `BootstrapPayload`. */
 ...
 * `0`. Port of `@cordierite/shared`'s `decodeBootstrap`.
 */
internal fun decodeCordieriteBootstrap(base64Url: String): CordieriteBootstrapPayload? {
 ...
 * URL(rawUrl).searchParams` does for the two params Cordierite cares about. Anything reading the
 * `cordierite` payload must stop at the first `&` -- naive slicing to end-of-string would swallow
...
/** True iff `rawUrl` includes a `cordierite` query parameter -- mirrors `deep-link-core.ts`'s
 * `hasCordieriteBootstrapQuery`. Does not itself decode or validate the payload. */
internal fun hasCordieriteBootstrapQuery(rawUrl: String?): Boolean {
    if (rawUrl.isNullOrEmpty()) return false
    return parseQueryParams(rawUrl).containsKey("cordierite")
}
...
internal sealed class BootstrapUrlParseError(message: String) : Exception(message) {
    class MissingPayload : BootstrapUrlParseError("Bootstrap URL is missing the cordierite query parameter.")
...
internal fun isCordieriteBootstrapExpired(
    expiresAt: Long,
    nowSeconds: Long,
): Boolean = expiresAt <= nowSeconds

/**
 * Parses and validates a Cordierite bootstrap deep link (PROTOCOL.md §2, `bootstrap.ts`'s
 * `parseBootstrapUrl`): the `cordierite` query param decodes to a valid, unexpired v2 payload,
 ...
 */
internal fun parseCordieriteBootstrapUrl(
    rawUrl: String,
    nowSeconds: Long,
    requirePrivateIp: Boolean,
): ParsedBootstrapUrl {
    val params = parseQueryParams(rawUrl)
    val rawPayload = params["cordierite"] ?: throw BootstrapUrlParseError.MissingPayload()

    val decoded = decodeCordieriteBootstrap(rawPayload) ?: throw BootstrapUrlParseError.InvalidPayload()

    if (isCordieriteBootstrapExpired(decoded.expiresAt, nowSeconds)) {
        throw BootstrapUrlParseError.ExpiredPayload()
    }

    if (requirePrivateIp && !isCordieriteLocalAddress(decoded.family, decoded.address)) {
...
/** Port of `@cordierite/shared`'s `isLocalAddress`: loopback/RFC1918 for IPv4, loopback/ULA/link-local
 * for IPv6. */
internal fun isCordieriteLocalAddress(
    family: Int,
    address: String,
): Boolean = if (family == 4) isLocalIpv4Address(address) else isCordieriteLocalIpv6Address(address)

private fun isCordieriteLocalIpv6Address(value: String): Boolean {
