...
/**
 * Dependency-free `ContentProvider` that exists for exactly one reason: to hand [Appduct] an
 ...
 * `Appduct.register(...)`, this has already run and [Appduct] has a client to register
 * against.
 *
 * Declared in this module's own `AndroidManifest.xml` with
 * `android:authorities="${applicationId}.appduct-init"` (per-app unique, as the platform
 ...
 * and what an app that removes it must do instead (call `Appduct.attach`-equivalent
 * initialization is not exposed publicly; removing this provider means driving `Appduct`
 * entirely without automatic init, which today means not using the `Appduct` facade at all).
 *
 * Not vendored into `@appduct/react-native` -- that package's
 ...
 * `AppductClient` directly and has its own initialization path
 * (`NativeAppductModule`'s constructor).
 */
class AppductInitProvider : ContentProvider() {
    override fun onCreate(): Boolean {
        val appContext = context?.applicationContext ?: return false
        Appduct.attach(appContext)
        return true
    }
