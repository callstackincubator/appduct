...
/**
 * Foreground/background observation seam backing [CordieriteClient]'s "pause reconnect while
 ...
 */
internal interface CordieriteAppLifecycleObserver {
 ...
 * still be a real Android process (this constructor is never exercised by [CordieriteClient]'s
 * unit tests, which inject [CordieriteNoopLifecycleObserver] instead).
 */
internal class CordieriteProcessLifecycleObserver(
    private val onBackgroundedChanged: (Boolean) -> Unit,
) : CordieriteAppLifecycleObserver,
...
/** Never reports backgrounded and never calls back -- used by [CordieriteClient]'s plain-JVM unit
 * tests, and by any host that manages its own foreground/background policy. */
internal class CordieriteNoopLifecycleObserver : CordieriteAppLifecycleObserver {
