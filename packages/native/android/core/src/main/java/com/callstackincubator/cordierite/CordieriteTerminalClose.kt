 ...
 */
internal const val CORDIERITE_POLICY_VIOLATION_CLOSE_CODE = 1000 + 8

/** Whether the daemon closed with a rejection that no retry of the same frame could ever satisfy. */
internal fun isCordieriteTerminalCloseCode(code: Int?): Boolean = code == CORDIERITE_POLICY_VIOLATION_CLOSE_CODE

/** The `sessionChange`-lost reason to report for a terminal close -- the daemon's own wire reason
 * when it sent one. */
internal fun cordieriteTerminalCloseReason(reason: String?): String = reason ?: "rejected_by_daemon"
