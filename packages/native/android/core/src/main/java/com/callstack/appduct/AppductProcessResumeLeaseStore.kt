...
internal data class AppductResumeEndpoint(
    val ip: String,
    val port: Int,
)

/** Immutable process-memory representation of the `ResumeLeaseV1` contract. */
internal data class AppductResumeLeaseV1(
    val sessionId: String,
    val resumeToken: String,
    val alias: String,
    val endpoint: AppductResumeEndpoint,
 ...
 */
internal object AppductProcessResumeLeaseStore {
    private data class Entry(
        val ownerGeneration: Long,
        val lease: AppductResumeLeaseV1,
    )
    ...
    fun get(): AppductResumeLeaseV1? = synchronized(lock) { entry?.lease }
...
internal fun commitSessionAck(
    ...
    ownerGeneration: Long,
    onAccepted: (AppductResumeLeaseV1) -> Unit,
    ...
    if (!AppductProcessResumeLeaseStore.replace(ownerGeneration, lease)) {
...
private fun parseSessionAckLease(
    ...
    return AppductResumeLeaseV1(
        ...
        alias = alias,
        endpoint = AppductResumeEndpoint(options.ip, options.port),
