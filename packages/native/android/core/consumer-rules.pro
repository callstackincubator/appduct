...
# into @appduct/react-native's android/core/ by scripts/sync-native-core.mjs, and consumed from
# there by every app that depends on @appduct/react-native (R8/ProGuard reads
# `consumerProguardFiles` from this library's AAR metadata, not just the app's own rules file).
#
# Keeps AppductNativeMarker's fully-qualified name unminified and unremoved so
