...
/**
 * No-op mirror of `core`'s [Cordierite] facade (docs/tasks/19-android-entry-points.md, issue #48
 * decision 2): same public surface, but every method does nothing and [state] is always
 * [ClientState.closed]. No `CordieriteInitProvider` in this module at all -- there is no `Context`
...
object Cordierite {
    ...
    fun addListener(listener: (CordieriteEvent) -> Unit): Subscription = Subscription {}
}
...
/** No-op mirror of `core`'s `CordieriteEvent` -- never emitted here, since [Cordierite.addListener]
