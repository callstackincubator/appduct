...
// Not vendored into @cordierite/react-native: `scripts/sync-native-core.mjs` only copies `Real/`,
// since the RN pod always compiles with `-DCORDIERITE_ENABLED` set (see `Cordierite.podspec`). This
// file exists purely so the `CordieriteCore` SwiftPM package itself compiles in a configuration
// that does not define `CORDIERITE_ENABLED` (a plain `swift build -c release`, and any future native
// SwiftPM consumer app that links this package into a Release build without the `AlwaysEnabled`
// trait) -- the exact declarations `CordieriteTurboBridge.swift` calls today, all no-ops, so code
// written against the real implementation still type-checks here unchanged.
//
// `CordieriteCoreMarker` (Real/CordieriteCoreMarker.swift) is deliberately NOT declared here: its
...
/// No-op mirror of the real `CordieriteConnectionManager`: every method matches the real actor's
...
/// same choice the JS `/noop` entry documents for `getCordieriteState()`.
actor CordieriteConnectionManager: NSObject {
  ...
  nonisolated func currentStateSnapshot() -> String {
    CordieriteConnectionState.idle.rawValue
  }
