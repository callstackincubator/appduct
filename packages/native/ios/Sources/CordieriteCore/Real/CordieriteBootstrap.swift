...
/// Ports `@cordierite/shared`'s `decodeBootstrap`/`isExpiredAt`/`isLocalAddress` and
/// `packages/react-native/src/bootstrap.ts`'s URL-layer wrapping (ARCHITECTURE.md §8) into Swift, so
/// `CordieriteClient.handleUrl` needs no JS round trip to judge a deep link.
public enum CordieriteAddressFamily: Int, Sendable, Equatable {
  case ipv4 = 4
  case ipv6 = 6
}

public struct CordieriteBootstrapPayload: Sendable, Equatable {
  public let family: CordieriteAddressFamily
...
public struct CordieriteBootstrapParseError: Error, Sendable, Equatable {
...
public func decodeCordieriteBootstrap(_ b64url: String) -> CordieriteBootstrapPayload? {
  ...
  let family: CordieriteAddressFamily = familyByte == familyIpv4 ? .ipv4 : .ipv6
  ...
  return CordieriteBootstrapPayload(
    family: family,
    address: address,
    port: port,
    sessionId: sessionId,
...
public func isCordieriteExpired(expiresAt: Int, now: Int) -> Bool {
  expiresAt <= now
}
...
public func isCordieriteLocalAddress(family: CordieriteAddressFamily, address: String) -> Bool {
  family == .ipv4 ? (isPrivateIpv4(address) || isLoopbackIpv4(address)) : isLocalIpv6(address)
}

/// True if `rawUrl` parses as a URL and includes a `cordierite` query parameter -- ports
/// `hasCordieriteBootstrapQuery` exactly.
public func hasCordieriteBootstrapQuery(_ rawUrl: String?) -> Bool {
  guard let rawUrl, let components = URLComponents(string: rawUrl) else { return false }
  return components.queryItems?.contains { $0.name == "cordierite" } ?? false
}

/// `sha256/` + 44 base64 chars (32-byte digest, standard alphabet incl. padding).
private let linkPinPattern = "^sha256/[A-Za-z0-9+/]{43}=$"

func extractCordieriteLinkPin(from components: URLComponents) -> String? {
...
/// Ports `parseBootstrapUrl` (via `parseBootstrapPayload`): decodes the `cordierite` query param,
/// checks expiry, optionally requires a private/loopback address, and folds in the sibling `pin`
/// param.
public func parseCordieriteBootstrapUrl(
  _ rawUrl: String,
  now: Int,
  requirePrivateIp: Bool
) throws -> CordieriteBootstrapPayload {
  guard let components = URLComponents(string: rawUrl) else {
    throw CordieriteBootstrapParseError(.invalidUrl, "Invalid bootstrap URL.")
  }

  guard let payload = components.queryItems?.first(where: { $0.name == "cordierite" })?.value, !payload.isEmpty else {
    throw CordieriteBootstrapParseError(.missingPayload, "Bootstrap URL is missing the cordierite query parameter.")
  }

  guard let decoded = decodeCordieriteBootstrap(payload) else {
    throw CordieriteBootstrapParseError(
      .invalidPayload,
      "Bootstrap payload must be a valid base64url-encoded v2 bootstrap blob (see Cordierite HANDSHAKE docs)."
    )
  }

  if isCordieriteExpired(expiresAt: decoded.expiresAt, now: now) {
    throw CordieriteBootstrapParseError(.expiredPayload, "Bootstrap payload has expired.")
  }

  if requirePrivateIp && !isCordieriteLocalAddress(family: decoded.family, address: decoded.address) {
    throw CordieriteBootstrapParseError(.invalidPayload, "Bootstrap payload is invalid.")
  }

  var result = decoded
  result.linkPin = extractCordieriteLinkPin(from: components)
  return result
}
