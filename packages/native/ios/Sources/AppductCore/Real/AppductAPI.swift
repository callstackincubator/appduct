...
// packages/native/README.md. (The RN bridge does not use this facade -- AppductTurboBridge.swift
// talks to AppductClient directly -- but the sync script copies the whole Real/ directory
// verbatim, so this file travels along for free and costs the RN pod nothing it doesn't already
// pay for AppductClient itself.)
#if APPDUCT_ENABLED
...
///
/// One instance is meant to live for the app process's lifetime; use `Appduct.shared` rather than
...
public final class Appduct: Sendable {
  /// First access constructs the underlying `AppductClient` and kicks off `restoreSession()` in
  ...
  /// through `Appduct.shared`, but `AppductCoreTests` (`@testable import AppductCore`)
  /// uses this initializer directly to wrap a scripted `FakeTransportSession`/`FakeClientTimers`
  /// pair instead of a real `AppductConnectionManager` -- see `AppductAPITests.swift`.
  init(client: AppductClient = AppductClient()) {
    self.client = client
  ...
  public func register(
    ...
    let coreHandler: ToolHandler = { args, context in
      ...
      return try Appduct.jsonValue(fromToolResult: result)
    }
  ...
  /// (see `AppductClient.postEvent`). `payload`, if present, must be convertible by the same rules
  /// as a tool result.
  public func postEvent(_ name: String, payload: Any? = nil) async throws {
    let jsonPayload: JSONValue?
    if let payload {
      guard let converted = try? Appduct.jsonValue(fromFoundation: payload) else {
        throw AppductJSONError("Appduct event payload is not JSON-serializable.")
      }
  ...
  /// Starts recovery from the native process-memory resume lease. `Appduct.shared`'s first access
...
/// Mirrors `AppductClientState` under the name the phase-3 API sketch (issue #48) uses. Not a
/// distinct type: renaming `AppductClientState` itself would ripple into `AppductClient`'s own
...
/// One unified event stream for `addListener`, folding `AppductClient`'s three separate listener
...
/// A handle returned by `Appduct.register`. `remove()` unregisters the tool; letting this value
...
/// A handle returned by `Appduct.addListener`. `cancel()` unsubscribes every channel it wired;
...
/// Backs `Subscription`: the three `AppductDisposable` tokens `addListener` collects arrive
/// asynchronously (each `AppductClient.onXChange` call is itself an actor hop), so this box lets
...
/// guarding all mutable state, the same pattern `AppductToolRegistryStore` and
/// `AppductProcessResumeLeaseStore` already use for the same reason.
final class SubscriptionBox: @unchecked Sendable {
  private let lock = NSLock()
  private var disposables: [any AppductDisposable] = []
  private var cancelled = false

  func add(_ disposable: any AppductDisposable) {
    lock.lock()
    if cancelled {
      lock.unlock()
      disposable.dispose()
...
private func makeToolDescriptor(
  ...
  func toJSONObject(_ dict: [String: Any]?, label: String) throws -> JSONObject? {
    ...
    guard let converted = try? Appduct.jsonValue(fromFoundation: dict), let object = converted.objectValue else {
...
extension Appduct {
  ...
  static func jsonValue(fromFoundation value: Any) throws -> JSONValue {
    ...
    case let dict as [String: Any]:
      ...
      return .object(out)
    default:
      throw AppductJSONError("Value of type \(type(of: value)) is not JSON-serializable.")
    }
  }

  /// Converts a tool handler's `Any?` return value to `JSONValue`, mapping a conversion failure to
  /// `tool_serialization_error` -- the exact wire error type `AppductClient`'s own
  /// `JSONValue.serialized()` check already produces for a `JSONValue` result that turns out
  /// non-finite (`respondSuccess`, `AppductClient+ToolInvocation.swift`) -- so a plain-app handler
