...
/// Mirrors `@appduct/shared`'s `ToolAnnotations` (PROTOCOL.md §7): exactly these three optional
...
/// Mirrors `@appduct/shared`'s `ToolDescriptor` (PROTOCOL.md §5/§7) one-to-one, including the
...
private let toolNamePattern: NSRegularExpression = {
  // `^[a-zA-Z0-9_-]{1,64}$` -- mirrors `@appduct/shared`'s `TOOL_NAME_PATTERN`.
