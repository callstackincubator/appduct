...
public struct AppductJSONError: Error, Sendable, CustomStringConvertible {
...
extension JSONValue {
  ...
  public static func parse(_ text: String) throws -> JSONValue {
    guard let data = text.data(using: .utf8) else {
      throw AppductJSONError("Not valid UTF-8 JSON text.")
    }
  ...
  /// `null` being explicitly disallowed for Appduct's own "must be JSON-serializable" checks --
  ...
  public func serialized() throws -> String {
    ...
    guard let text = String(data: data, encoding: .utf8) else {
      throw AppductJSONError("Failed to encode JSON as UTF-8.")
    }
    return text
  }
  ...
  private func assertFinite() throws {
    ...
    case .number(let value):
      guard value.isFinite else {
        throw AppductJSONError("JSON number must be finite (got \(value)).")
      }
    case .array(let values):
