...
Pod::Spec.new do |s|
  s.name           = 'CordieriteCore'
  # Version is single-sourced from @cordierite/react-native's package.json (see
  ...
  s.summary        = 'Framework-free Swift core for Cordierite: TLS-pinned session transport and resume leases.'
  s.description    = 'The native connection layer behind @cordierite/react-native, usable directly ' \
  ...
  s.source_files = 'ios/Sources/CordieriteCore/Real/**/*.swift'

  s.pod_target_xcconfig = {
    'DEFINES_MODULE' => 'YES',
    # Unconditional, unlike the SwiftPM target's configuration-gated define: a CocoaPods consumer
    # (this repo's own @cordierite/react-native, or any other pod depending on this one) gates
    ...
    # CordieriteConnectionManager); `-strict-concurrency=complete` keeps it that way by failing the
  ...
  s.test_spec 'Tests' do |test_spec|
    ...
    # packages/cordierite/src/spki-pin.ts) that need only Foundation/Security.
    test_spec.source_files = 'ios/Tests/CordieriteCoreTests/**/*.swift'
