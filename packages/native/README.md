...
Native dependency. It also ships a small public facade (`Cordierite.shared` on iOS, the
`Cordierite` object on Android) that a plain native app calls directly, with no React Native or
...
- **Building a plain Android app?** See [`android/README.md`](android/README.md) — install via
  Maven, the `cordieriteScheme` manifest placeholder, registering tools, hardened-build config.
- **Building a React Native app?** Use [`@cordierite/react-native`](../react-native/README.md)
...
For how this directory is laid out, how `@cordierite/react-native` vendors it, the facade-exclusion
