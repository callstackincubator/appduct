...
@objc(AppductTurboBridge)
public final class AppductTurboBridge: NSObject, @unchecked Sendable {
  ...
  @objc public func wireEventHandlers(
    ...
    Task {
      ...
      _ = await self.client.onError { event in
        error(AppductTurboBridge.errorPayload(event))
      }
    }
  }

  private static func errorPayload(_ event: AppductUnifiedErrorEvent) -> NSDictionary {
  ...
  private func makeBridgeHandler() -> ToolHandler {
    { [weak self] args, context in
      guard let self else {
        throw AppductToolHandlerError(type: "tool_execution_error", message: "Appduct bridge was deallocated.")
      }
  ...
  @objc public func respondToToolCall(id: NSString, resultJson: NSString?, errorJson: NSString?) {
    ...
    if let errorJson {
      guard
        ...
        pendingToolCalls.reject(
          id: callId,
          error: AppductToolHandlerError(type: "tool_execution_error", message: "Malformed tool_error from JS.")
        )
        return
      }
      pendingToolCalls.reject(id: callId, error: AppductToolHandlerError(type: type, message: message, details: object["details"]))
      return
    }
    ...
    do {
      ...
      pendingToolCalls.reject(
        id: callId,
        error: AppductToolHandlerError(type: "tool_serialization_error", message: "Malformed tool_result JSON from JS.")
      )
      return
    }
  ...
  @objc public func connect(
    ...
    Task {
      do {
        ...
        let input = try parseAppductConnectInput(value)
        try await self.client.connect(input, supersede: supersede)
        resolve(nil)
      } catch {
        reject("E_APPDUCT", AppductTurboBridge.describe(error), error)
      }
    }
  }
    ...
    Task {
      do {
        ...
        reject(
          ...
          AppductClient.AppductNotActiveError()
        )
      } catch {
        reject("E_APPDUCT", AppductTurboBridge.describe(error), error)
      }
  ...
  private static func describe(_ error: Error) -> String {
    if let handlerError = error as? AppductToolHandlerError { return handlerError.message }
  ...
  /// Purely a `Bundle.main` read (`currentAppductBuildConfig()`), so no actor hop is needed.
  ...
  /// deliberately-not-blocking tradeoff `AppductConnectionManager.invalidate()` documents.
...
final class PendingToolCallStore: @unchecked Sendable {
  ...
  func reject(id: String, error: AppductToolHandlerError) {
