...
# Only evaluated when autolinking has already decided to link Appduct, so the line appearing at
# all is the proof. Ungated: a release build carrying Appduct by mistake is the failure worth
# catching, and this is grep-able in CI output. CocoaPods evaluates the podspec several times per
# install, hence the guard.
if defined?(Pod::UI) && !defined?($appduct_banner_shown)
  $appduct_banner_shown = true
  Pod::UI.puts "[appduct] native module INCLUDED in this build (v#{package['version']})"
end
...
Pod::Spec.new do |s|
  ...
  s.pod_target_xcconfig = {
    ...
    # sources are compiled the decision to include Appduct has already been made -- the vendored
    # ios/Core sources should always compile as the real implementation, never the
    # AppductCore SwiftPM package's Stub/ branch (which this pod never even sees -- only Real/ is
    # vendored). All connection state lives behind a single actor (see AppductConnectionManager);
