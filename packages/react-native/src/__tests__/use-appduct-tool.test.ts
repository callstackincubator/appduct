 ...
 * one contract `useAppductTool` depends on (stable ref slots in call order, run the effect on
...
const toolDefinition = (
  name = "t",
): AppductToolRegistration<undefined, undefined> => ({
  name,
...
const fakeContext = {} as AppductToolExecutionContext;
...
const schemaExporting = (
  ...
  }) as unknown as AppductRuntimeSchema;

/** A Standard Schema with no JSON Schema exporter -- the zod 3 / plain valibot shape. */
const shapelessSchema = (): AppductRuntimeSchema =>
  ({
    "~standard": {
      version: 1,
      vendor: "test",
...
describe("createUseAppductTool", () => {
  ...
  test("does not re-register across re-renders with unchanged deps", async () => {
    ...
    host.render(() => useAppductTool(toolDefinition(), [1]));
    host.render(() => useAppductTool(toolDefinition(), [1]));
  ...
  test("disposes the old registration and creates a new one when deps change (fast-refresh churn)", async () => {
    ...
    host.render(() => useAppductTool(toolDefinition(), [1]));
    host.render(() => useAppductTool(toolDefinition(), [2]));
  ...
  describe("derived registration key (deps omitted)", () => {
    test("N re-renders with an unchanged definition produce exactly one registration and no removals", async () => {
      ...
      // inline `useAppductTool({ ... })` call inside a component body produces.
    ...
    test("changing description re-registers (remove + upsert)", async () => {
      ...
      host.render(() =>
        useAppductTool({ ...toolDefinition(), description: "before" }),
      );
      host.render(() =>
        useAppductTool({ ...toolDefinition(), description: "after" }),
      );
    ...
    test("changing name re-registers under the new name", async () => {
      ...
      host.render(() => useAppductTool(toolDefinition("before")));
      host.render(() => useAppductTool(toolDefinition("after")));
    ...
    test("a schema that exports no JSON Schema still re-registers when it is added, swapped or removed", async () => {
      ...
      let outputSchema: AppductRuntimeSchema | undefined;
      const render = () =>
        host.render(() =>
          useAppductTool({
            name: "t",
    ...
    test("a null deps argument (untyped JS callers) behaves like an omitted one", async () => {
      ...
      host.render(() => useAppductTool(toolDefinition(), nullDeps));
      host.render(() => useAppductTool(toolDefinition(), nullDeps));
      expect(registrations).toHaveLength(1);
  ...
  describe("inert entry: no JSON Schema exporter injected", () => {
    ...
    test("still honours options.enabled", async () => {
      ...
      host.render(() =>
        useAppductTool(toolDefinition(), undefined, { enabled: true }),
      );
      expect(registrations).toHaveLength(1);

      host.render(() =>
        useAppductTool(toolDefinition(), undefined, { enabled: false }),
      );
  ...
  test("a later call always sees the latest definition object, not a stale closure", async () => {
    ...
    host.render(() => useAppductTool(toolDefinition("first"), [1]));
    host.render(() => useAppductTool(toolDefinition("second"), [2]));
  ...
  describe("options.enabled", () => {
    ...
    test("enabled: false never registers", async () => {
      ...
      host.render(() =>
        useAppductTool(toolDefinition(), [], { enabled: false }),
      );
    ...
    test("true -> false removes the registration and leaks nothing", async () => {
      ...
      host.render(() =>
        useAppductTool(toolDefinition(), [], { enabled: true }),
      );
      expect(registrations).toHaveLength(1);
      expect(registrations[0]?.removed).toBe(false);
  ...
  describe("identity, not name: a stale disposer never removes a newer registration under the same name", () => {
     ...
     * `useAppductTool` hook instances instead of calling the registry directly.
     */
    ...
    test("disabling hook A never removes hook B's still-active registration under the same name", async () => {
      ...
      hostA.render(() =>
        useAppductToolA(toolDefinition("shared"), [], { enabled: true }),
      );
      ...
      hostB.render(() =>
        useAppductToolB(toolDefinition("shared"), [], { enabled: true }),
      );
      ...
      hostA.render(() =>
        useAppductToolA(toolDefinition("shared"), [], { enabled: false }),
      );
      ...
      hostB.render(() =>
        useAppductToolB(toolDefinition("shared"), [], { enabled: false }),
      );
