 ...
 */
export type AppductBuildConfigNative = {
  trust: string;
  hasEmbeddedPins: boolean;
  allowPrivateLanOnly: boolean;
};

/** Mirrors `AppductUnifiedErrorEvent` in `Appduct.types.ts` minus `cause` (not serializable). */
export type AppductErrorEventNative = {
...
/** Mirrors `AppductUnifiedStateChangeEvent`: `state` is a `AppductClientState`. */
export type AppductStateChangeEventNative = {
  state: string;
  reason?: string;
};

/** Mirrors `AppductSessionChangeEvent`. `sessionId`/`alias` are both null once the session is
 ...
 * reason). */
export type AppductSessionChangeEventNative = {
  type: string;
...
export type AppductToolCallEventNative = {
  id: string;
  name: string;
...
export type AppductToolCancelEventNative = {
  id: string;
...
export interface Spec extends TurboModule {
  ...
  /**
   * Programmatic connect. `inputJson` is a `AppductConnectInput` (a decoded bootstrap payload
  ...
  /** A `AppductClientState`: "idle" | "connecting" | "active" | "reconnecting" | "closed". */
  getState(): string;
  ...
  readonly onToolCall: CodegenTypes.EventEmitter<AppductToolCallEventNative>;
  readonly onToolCancel: CodegenTypes.EventEmitter<AppductToolCancelEventNative>;
  readonly onStateChange: CodegenTypes.EventEmitter<AppductStateChangeEventNative>;
  readonly onSessionChange: CodegenTypes.EventEmitter<AppductSessionChangeEventNative>;
  readonly onError: CodegenTypes.EventEmitter<AppductErrorEventNative>;
}

export const NativeAppduct =
  TurboModuleRegistry.getEnforcing<Spec>("Appduct");
