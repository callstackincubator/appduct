import { installAppductDeepLinkBootstrap } from "./deep-link-install";
import { appductClient, noopIfNativeUnavailable } from "./default-client";

/**
 * `@appduct/react-native/auto` — side-effect entry (ARCHITECTURE.md §11). Importing this module
 * subscribes the default client to runtime deep links and starts native-lease recovery before the
 * initial URL is considered: `import "@appduct/react-native/auto";` once, near your app's entry
 * point.
 *
 * To control *when* that happens — behind `__DEV__`, a QA-build toggle, or after other startup
 * work — `require("@appduct/react-native/auto")` at that point instead; Metro resolves it
...
noopIfNativeUnavailable(
  () => installAppductDeepLinkBootstrap(appductClient),
  () => {},
);
