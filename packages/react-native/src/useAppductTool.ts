...
export type UseAppductToolOptions = {
   ...
   * `{ enabled: process.env.EXPO_PUBLIC_APPDUCT_TOOLS === "full" }`. See `docs/SECURITY.md`'s
...
/** Options for `createUseAppductTool` (library-internal, not part of the public API). */
export type CreateUseAppductToolOptions = {
  /**
   ...
   * whose whole purpose is to carry no Appduct work. Without it the effect keys off `enabled`
 ...
 */
type SchemaKeyMemo = {
  schema: AppductRuntimeSchema | undefined;
...
/**
 * Builds `useAppductTool` on top of a `registerTool` implementation. Both the real (`.`) and
 ...
 */
export function createUseAppductTool(
  registerTool: ToolRegistrar,
  { exportSchema }: CreateUseAppductToolOptions = {},
) {
  /**
   ...
   */
  return function useAppductTool<
    TInputSchema extends AppductRuntimeSchema | undefined,
    TOutputSchema extends AppductRuntimeSchema | undefined,
  >(
