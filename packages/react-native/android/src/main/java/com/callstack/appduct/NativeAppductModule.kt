 ...
 * always propagate).
 */
@ReactModule(name = NativeAppductSpec.NAME)
class NativeAppductModule(
    reactContext: ReactApplicationContext,
) : NativeAppductSpec(reactContext) {
    /** Everything here is fire-and-forget bridging between the TurboModule call and
     * [AppductClient]'s own suspend API -- [AppductClient] internally confines its real state
    ...
    private val client = AppductClient(reactContext)
    ...
    override fun registerTool(descriptorJson: String) {
        val descriptor = AppductToolDescriptor.fromJson(descriptorJson)
        // Throws AppductInvalidToolDescriptorException synchronously for an invalid descriptor,
        ...
        client.registerTool(descriptor) { args, context ->
            ...
            try {
                ...
                if (errorJson != null) {
                    ...
                    throw AppductToolReplyError(
                        errorType = errorObj.optString("type", "tool_execution_error"),
                        message = errorObj.optString("message", "Appduct tool execution failed."),
    ...
    override fun connect(
        ...
        val input =
            try {
                AppductConnectInput.fromJson(inputJson)
            } catch (e: Exception) {
                promise.reject("E_APPDUCT", e.message, e)
                return
            }
    ...
    override fun postEvent(
        ...
        moduleScope.launch {
            // [AppductClient.postEvent] is itself best-effort (silently a no-op while no session
            ...
            // the iOS bridge's `E_APPDUCT_NOT_ACTIVE` (`AppductClient.AppductNotActiveError`).
            if (client.state.name != "active" || client.sessionId == null) {
                promise.reject(
                    "E_APPDUCT_NOT_ACTIVE",
    ...
    // `final` implementation on `NativeAppductSpec` that validates and forwards this method's
    // return value instead -- see the generated `NativeAppductSpec.getConstants()`.
...
// AppductToolDescriptor.fromJson / AppductConnectInput.fromJson (packages/native/android/core
// and core-noop's AppductClientTypes.kt) -- vendored like the Swift equivalents
// (parseToolDescriptor/parseAppductConnectInput) and covered by FixturesConformanceTest.kt.
