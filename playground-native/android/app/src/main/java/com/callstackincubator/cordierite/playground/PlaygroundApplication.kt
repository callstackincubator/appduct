...
import com.callstackincubator.cordierite.CordieriteEvent
import com.callstackincubator.cordierite.ToolAnnotations
import kotlinx.coroutines.delay
import org.json.JSONObject

/**
 * Registers the same tool set as the Expo playground (`playground/app/(tabs)/index.tsx`) so the
 * two can be driven identically from `cordierite tools`/`invoke` (docs/tasks/19-android-entry-points.md).
 * By the time [onCreate] runs, [com.callstackincubator.cordierite.CordieriteInitProvider] has
 * already captured this process's application `Context` and started lease recovery -- nothing
 * else needs to happen before [Cordierite.register] works.
 *
 * On a release build (`releaseImplementation` resolves `core-noop`, see `app/build.gradle`),
 * every call below still compiles and runs, but does nothing: `Cordierite.register` returns an
 * inert [com.callstackincubator.cordierite.ToolRegistration] and the listener below is never
 * invoked, matching every other Cordierite consumer's compiled-out release behavior
