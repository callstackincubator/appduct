...
/// `AppductCore` itself, and both of those SwiftUI APIs need iOS 16.
struct ContentView: View {
  ...
  var body: some View {
    NavigationView {
      ...
      .task {
        // Started here, not from `AppductPlaygroundApp.init()`: `PlaygroundViewModel` is
  ...
  private func postEvent() {
    ...
    Task { @MainActor in
      ...
      do {
        try await Appduct.shared.postEvent(
