...
let package = Package(
  ...
  products: [
    .library(name: "AppductCore", targets: ["AppductCore"])
  ],
  traits: [
  ...
  targets: [
    .target(
      name: "AppductCore",
      path: "packages/native/ios/Sources/AppductCore",
      swiftSettings: [
        ...
        .define("APPDUCT_ENABLED", .when(configuration: .debug)),
        ...
        .define("APPDUCT_ENABLED", .when(traits: ["AlwaysEnabled"])),
      ]
    ),
    .testTarget(
      name: "AppductCoreTests",
      dependencies: ["AppductCore"],
      path: "packages/native/ios/Tests/AppductCoreTests"
    ),
  ]
)
